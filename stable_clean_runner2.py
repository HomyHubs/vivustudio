...
def restore_scene(
    ...
    with torch.inference_mode():
        for index in anchors:
            restored[index] = lama_restore(crops[index], model, mask_tensor, ring)
    ...
    for left_anchor, right_anchor in zip(anchors, anchors[1:]):
        ...
        for index in range(left_anchor + 1, right_anchor):
            ...
            # Fast motion: when neither warp direction is trustworthy, don't
            # smear a bad warp -- inpaint this frame directly with LaMa. The crop
            # is only 256x256, so the extra inference is cheap.
            if max(forward_confidence, backward_confidence) < fallback_confidence:
                with torch.inference_mode():
                    result[index] = lama_restore(
                        crops[index], model, mask_tensor, ring
                    )
                continue
