...
class GeminiLogoWorker(QObject):
    ...
    def _run_propainter_temporal(
        self, box: tuple[int, int, int, int], codec: str
    ) -> None:
        import cv2

        processing_source = (
            self.alpha_output
            if Path(self.alpha_output).is_file()
            else self.source
        )

        repo = APP_ROOT / "vendor" / "ProPainter"
        script = repo / "inference_propainter.py"
        weights = repo / "weights"
        if not script.is_file():
            raise RuntimeError(
                "AI Temporal is not ready; the ProPainter component is missing:\n"
                + str(script)
            )
        checkpoint_paths = [
        ...
        first_download = any(not path.is_file() for path in checkpoint_paths)

        capture = cv2.VideoCapture(processing_source)
        if not capture.isOpened():
            raise RuntimeError("Could not open the original video for AI Temporal.")
        frame_width = int(capture.get(cv2.CAP_PROP_FRAME_WIDTH))
        frame_height = int(capture.get(cv2.CAP_PROP_FRAME_HEIGHT))
        fps = float(capture.get(cv2.CAP_PROP_FPS) or 30.0)
        frame_count = max(1, int(capture.get(cv2.CAP_PROP_FRAME_COUNT)))
        capture.release()
        self.progress.emit(79, "AI Temporal · detecting scene cuts...")
        scene_ranges = detect_video_scene_ranges(self.source, frame_count)
        roi_x, roi_y, roi_w, roi_h = gemini_temporal_roi(
            frame_width, frame_height, box, side=384
        )
        x, y, box_w, box_h = box
        roi_box = (x - roi_x, y - roi_y, box_w, box_h)

        temp_parent = config_dir() / "tools_temp"
        temp_parent.mkdir(parents=True, exist_ok=True)
        with tempfile.TemporaryDirectory(prefix="propainter_", dir=str(temp_parent)) as temp:
            work = Path(temp)
            roi_video = work / "logo_roi.mp4"
            exact_mask_path = work / "logo_mask_exact.png"
            mask_path = work / "logo_mask.png"
            feather_path = work / "logo_mask_feather.png"
            create_gemini_shape_mask(roi_w, roi_h, roi_box, exact_mask_path)
            halo_padding = max(5, min(12, round(max(box_w, box_h) * 0.10)))
            create_expanded_gemini_mask(
                exact_mask_path, mask_path, padding=halo_padding
            )
            create_feathered_gemini_mask(mask_path, feather_path)

            self.progress.emit(
                80,
                f"AI Temporal · ROI {roi_w}×{roi_h}, {len(scene_ranges)} scene(s), "
                f"halo {halo_padding}px",
            )
            crop_command = [
                ffmpeg_executable(), "-y", "-hide_banner", "-loglevel", "error",
                "-i", processing_source,
                ...
                "-pix_fmt", "yuv420p", str(roi_video),
            ]
            self.process = subprocess.Popen(
                crop_command, stdout=subprocess.DEVNULL, stderr=subprocess.PIPE,
                creationflags=0x08000000 if sys.platform == "win32" else 0,
            )
            _stdout, crop_stderr = self.process.communicate()
            if self._cancelled:
                raise RuntimeError("AI Temporal was stopped.")
            if self.process.returncode or not roi_video.is_file():
                details = crop_stderr.decode("utf-8", errors="replace") if crop_stderr else ""
                raise RuntimeError("Could not create the ROI for AI Temporal:\n" + details[-3000:])

            bundled_python = APP_ROOT / ".conda-env" / (
                "python.exe" if sys.platform == "win32" else "bin/python"
            )
            python_executable = str(bundled_python if bundled_python.is_file() else Path(sys.executable))
            environment = os.environ.copy()
            environment["PYTHONUNBUFFERED"] = "1"
            environment.setdefault("CUDA_MODULE_LOADING", "LAZY")
            environment.setdefault("PYTORCH_CUDA_ALLOC_CONF", "expandable_segments:True")
            segment_videos: list[Path] = []
            if len(scene_ranges) == 1:
                segment_videos.append(roi_video)
            else:
                for scene_index, (start_frame, end_frame) in enumerate(scene_ranges):
                    segment = work / f"scene_{scene_index:04d}.mp4"
                    split_command = [
                        ffmpeg_executable(), "-y", "-hide_banner", "-loglevel", "error",
                        "-i", str(roi_video),
                        "-vf", (
                        ...
                        "-crf", "0", "-pix_fmt", "yuv420p", str(segment),
                    ]
                    split_result = subprocess.run(
                        split_command, capture_output=True,
                        creationflags=0x08000000 if sys.platform == "win32" else 0,
                    )
                    if split_result.returncode or not segment.is_file():
                        details = split_result.stderr.decode("utf-8", errors="replace")
                        raise RuntimeError("Could not split the AI Temporal scene:\n" + details[-3000:])
                    segment_videos.append(segment)

            result_videos: list[Path] = []
            for scene_index, segment_video in enumerate(segment_videos):
                result_root = work / f"results_{scene_index:04d}"
                scene_number = scene_index + 1
                self.progress.emit(
                    83 + round(scene_index * 11 / max(1, len(segment_videos))),
                    "AI Temporal · downloading the official model for the first run (~200 MB)..."
                    if first_download and scene_index == 0 else
                    f"AI Temporal · processing scene {scene_number}/{len(segment_videos)} after Premium Safe...",
                )
                result_video = result_root / segment_video.stem / "inpaint_out.mp4"
                details = ""
                return_code: int | None = None
                for attempt in range(2):
                    if result_root.exists():
                        shutil.rmtree(result_root, ignore_errors=True)
                    safe_retry = attempt == 1
                    command = [
                        python_executable, str(script),
                        "--video", str(segment_video),
                        "--mask", str(mask_path),
                        "--output", str(result_root),
                        "--subvideo_length", "36" if safe_retry else "50",
                        "--neighbor_length", "6" if safe_retry else "10",
                        "--ref_stride", "12" if safe_retry else "10",
                        "--raft_iter", "8" if safe_retry else "12",
                        "--mask_dilation", "1",
                        "--fp16",
                    ]
                    if safe_retry:
                        self.progress.emit(
                            83 + round(scene_index * 11 / max(1, len(segment_videos))),
                            f"AI Temporal · scene {scene_number} was interrupted, "
                            "retrying in GPU safe mode...",
                        )
                        time.sleep(0.8)
                    self.process = subprocess.Popen(
                    ...
                    if self._cancelled:
                        raise RuntimeError("AI Temporal was stopped.")
                    details = "".join(lines)[-6000:]
                    if return_code == 0 and result_video.is_file():
                        break
                if return_code != 0 or not result_video.is_file():
                    if "out of memory" in details.lower():
                        details += "\nClose other applications using the GPU, then try again."
                    if not details.strip():
                        details = "The GPU subprocess ended without returning a traceback."
                    raise RuntimeError(
                        f"AI Temporal failed on scene {scene_number} "
                        f"after two attempts (exit code {return_code}):\n" + details
                    )
            ...
            if len(result_videos) == 1:
                ...
                concat_list = work / "scene_results.txt"
                concat_list.write_text(
                    "".join(f"file '{path.as_posix()}'\n" for path in result_videos),
                    encoding="utf-8",
                )
                result_video = work / "scene_aware_roi.mp4"
                concat_result = subprocess.run(
                    [
                        ffmpeg_executable(), "-y", "-hide_banner", "-loglevel", "error",
                        "-f", "concat", "-safe", "0", "-i", str(concat_list),
                        "-an", "-c:v", "libx264", "-preset", "ultrafast",
                        "-crf", "0", "-pix_fmt", "yuv420p", str(result_video),
                    ],
                    capture_output=True,
                    creationflags=0x08000000 if sys.platform == "win32" else 0,
                )
                if concat_result.returncode or not result_video.is_file():
                    details = concat_result.stderr.decode("utf-8", errors="replace")
                    raise RuntimeError("Could not concatenate the scene results:\n" + details[-3000:])

            self.progress.emit(96, "AI Temporal · compositing scene ROIs into the original video...")

            filter_complex = (
                "[1:v]format=rgba[patch];"
                "[2:v]format=gray[mask];"
                "[patch][mask]alphamerge[patcha];"
                f"[0:v][patcha]overlay={roi_x}:{roi_y}:shortest=1[v]"
            )
            merge_command = [
                ffmpeg_executable(), "-y", "-hide_banner", "-loglevel", "error",
                "-i", self.source, "-i", str(result_video),
                "-loop", "1", "-framerate", f"{fps:.6f}", "-i", str(feather_path),
                "-filter_complex", filter_complex,
                "-map", "[v]", "-map", "0:a?", "-c:v", codec,
            ]
            if codec in {"h264_nvenc", "hevc_nvenc"}:
                merge_command.extend(["-preset", "p4", "-cq", "18"])
            elif codec == "h264_qsv":
                merge_command.extend(["-global_quality", "18"])
            elif codec == "h264_amf":
                merge_command.extend(["-quality", "quality"])
            merge_command.extend([
                "-c:a", "copy", "-movflags", "+faststart", "-shortest",
                "-progress", "pipe:1", "-nostats", self.temporal_output,
            ])
            self.process = subprocess.Popen(
                merge_command, stdout=subprocess.PIPE, stderr=subprocess.STDOUT,
            ...
            for line in self.process.stdout:
                ...
                if len(merge_lines) > 300:
                    merge_lines.pop(0)
            self.process.wait()
            if self._cancelled:
                raise RuntimeError("AI Temporal was stopped.")
            if self.process.returncode or not Path(self.temporal_output).is_file():
                raise RuntimeError(
                    "Could not composite AI Temporal into the original video:\n"
                    + "".join(merge_lines)[-4000:]
                )

    def _run_vsr_propainter(self, box: tuple[int, int, int, int]) -> None:
        import cv2

        codec = available_video_gpu_codec() or "libx264"
        repo = APP_ROOT / "vendor" / "video-subtitle-remover"
        runner = APP_ROOT / "vsr_propainter_runner.py"
        model_dir = APP_ROOT / "vendor" / "ProPainter" / "weights"
        if not runner.is_file() or not (model_dir / "ProPainter.pth").is_file():
            raise RuntimeError(
                "VSR ProPainter is not ready; a component or checkpoint is missing."
            )
        capture = cv2.VideoCapture(self.source)
        if not capture.isOpened():
            raise RuntimeError("Could not open the original video for VSR ProPainter.")
        frame_count = max(1, int(capture.get(cv2.CAP_PROP_FRAME_COUNT)))
        capture.release()
        self.progress.emit(79, "VSR ProPainter · detecting scene cuts...")
        scene_ranges = detect_video_scene_ranges(self.source, frame_count)
        ...
        temp_parent = config_dir() / "tools_temp"
        temp_parent.mkdir(parents=True, exist_ok=True)
        with tempfile.TemporaryDirectory(prefix="vsr_sttn_", dir=str(temp_parent)) as temp:
            silent_output = Path(temp) / "sttn_video.mp4"
            command = [
            ...
            self.progress.emit(
                80,
                f"VSR ProPainter · rectangular mask +10px · {len(scene_ranges)} scene(s)",
            )
            self.process = subprocess.Popen(
            ...
            for line in self.process.stdout:
                ...
                if match:
                    ...
                    percent = 80 + min(17, round(done * 17 / total))
                    self.progress.emit(
                        percent, f"VSR ProPainter · {done}/{total} frame"
                    )
            self.process.wait()
            if self._cancelled:
                raise RuntimeError("VSR ProPainter was stopped.")
            if self.process.returncode or not silent_output.is_file():
                details = "".join(lines)[-6000:]
                raise RuntimeError(
                    f"VSR ProPainter failed (exit code {self.process.returncode}):\n"
                    + details
                )

            self.progress.emit(98, "VSR ProPainter · preserving original audio...")
            ...
            if remux.returncode or not output.is_file():
                ...
                raise RuntimeError("Could not mux the VSR ProPainter audio:\n" + details[-3000:])
...
class MainWindow(QMainWindow):
    ...
    def on_tools_completed(
        ...
        recommended = {
            "stable": "Stable Clean",
            "temporal": "VSR ProPainter (Experimental)",
            "lama": "LaMa Fallback",
