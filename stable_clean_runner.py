...
def restore_scene(
    ...
    fallback_confidence: float = 0.25, temporal_weight: float = 0.6,
    temporal_radius: int = 2,
) -> list[np.ndarray]:
    """Guarantee logo removal on every frame, then de-flicker temporally.

    LaMa is run *directly on every frame*, so the masked region is always
    inpainted -- the logo can never survive regardless of how the flow behaves.

    Flicker (per-frame LaMa producing slightly different fills, most visible on
    fast motion) is then suppressed with a symmetric, motion-compensated
    temporal average: each neighbour's already-clean patch is warped onto the
    current frame and averaged in. The contribution ramps smoothly with warp
    confidence (no hard on/off switch that would itself flicker) and decays with
    distance. Because every averaged patch is already logo-free, an imperfect
    warp can only soften texture slightly -- it can never reintroduce the logo.
    """
    n = len(crops)
    if n == 0:
        return []
    ...
    if temporal_weight <= 0.0 or temporal_radius <= 0:
        return direct

    # 2) Motion-compensated temporal averaging for de-flicker.
    confidence_span = max(1e-3, 1.0 - fallback_confidence)
    result: list[np.ndarray] = []
    for index in range(n):
        accumulator = direct[index].astype(np.float32).copy()
        weight_sum = 1.0
        for offset in range(1, temporal_radius + 1):
            for neighbor in (index - offset, index + offset):
                ...
                warped, confidence = warp_to_target(
                    direct[neighbor], crops[neighbor], crops[index], hard, ring
                )
                # Smooth ramp: 0 at/below fallback_confidence, up to
                # temporal_weight at perfect alignment; closer frames weigh more.
                ramp = max(0.0, (confidence - fallback_confidence) / confidence_span)
                weight = temporal_weight * ramp / offset
                if weight <= 0.0:
                    continue
                accumulator += warped.astype(np.float32) * weight
                weight_sum += weight
        result.append((accumulator / weight_sum).clip(0, 255).astype(np.uint8))
    return result
...
def main() -> int:
    ...
    parser.add_argument("--temporal-weight", type=float, default=0.6)
    ...
    mask_tensor = torch.from_numpy(hard[None, None]).to(device)
    ...
    try:
        for scene_start, scene_end in ranges:
            ...
            clean_crops = restore_scene(
                ...
                args.temporal_weight, args.temporal_radius,
            )
            capture.set(cv2.CAP_PROP_POS_FRAMES, scene_start)
