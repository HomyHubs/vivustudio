...
def build_masks(
    ...
    if asset is not None and x2 > x1 and y2 > y1:
        ...
        kernel_size = padding_target * 2 + 1
        ...
        padding = max(5, round(max(box_w, box_h) * 0.12 * target / crop_w))
        hard[
            max(0, y1 - padding):min(target, y2 + padding),
            max(0, x1 - padding):min(target, x2 + padding),
        ] = 1.0
    feather = cv2.GaussianBlur(hard, (0, 0), 1.8)[:, :, None]
...
def motion_score(previous: np.ndarray, current: np.ndarray, ring: np.ndarray) -> float:
    ...
    values = gray[ring]
    return float(np.median(values) / 255.0) if values.size else 0.0
...
def choose_anchors(
    ...
    anchors.update(range(0, len(crops), max(3, stride)))
    last_dynamic = 0
    for index in range(1, len(crops)):
        # Add a fresh LaMa observation when a foreground enters/leaves the
        # watermark neighborhood instead of forcing flow through an occlusion.
        if index - last_dynamic >= 3 and motion_score(crops[index - 1], crops[index], ring) >= 0.055:
            anchors.add(index)
            last_dynamic = index
...
def warp_to_target(
    ...
    flow = cv2.calcOpticalFlowFarneback(
        target_gray, source_gray, None, 0.5, 4, 19, 4, 7, 1.5, 0
    )
    ring_flow = flow[ring]
    if ring_flow.size:
        local_motion = np.median(ring_flow, axis=0)
...
def restore_scene(
    ...
    anchors = choose_anchors(crops, ring, anchor_stride)
    restored: dict[int, np.ndarray] = {}
    with torch.inference_mode():
        for index in anchors:
            rgb = cv2.cvtColor(crops[index], cv2.COLOR_BGR2RGB).astype(np.float32) / 255.0
            ...
            restored[index] = match_local_color(prediction, crops[index], ring)

    result: list[np.ndarray | None] = [None] * len(crops)
    for anchor in anchors:
        result[anchor] = restored[anchor]
    for left_anchor, right_anchor in zip(anchors, anchors[1:]):
        forward: dict[int, tuple[np.ndarray, float]] = {
            left_anchor: (restored[left_anchor], 1.0)
        }
        current = restored[left_anchor]
        confidence = 1.0
        for index in range(left_anchor + 1, right_anchor):
            current, step_confidence = warp_to_target(
                current, crops[index - 1], crops[index], hard, ring
            )
            confidence *= step_confidence
            forward[index] = (current, confidence)

        backward: dict[int, tuple[np.ndarray, float]] = {
            right_anchor: (restored[right_anchor], 1.0)
        }
        current = restored[right_anchor]
        confidence = 1.0
        for index in range(right_anchor - 1, left_anchor, -1):
            current, step_confidence = warp_to_target(
                current, crops[index + 1], crops[index], hard, ring
            )
            confidence *= step_confidence
            backward[index] = (current, confidence)

        span = max(1, right_anchor - left_anchor)
        for index in range(left_anchor + 1, right_anchor):
            position = (index - left_anchor) / span
            forward_frame, forward_confidence = forward[index]
            backward_frame, backward_confidence = backward[index]
            forward_weight = (1.0 - position) * forward_confidence
            backward_weight = position * backward_confidence
            total = max(1e-5, forward_weight + backward_weight)
            clean = (
                forward_frame.astype(np.float32) * (forward_weight / total)
                + backward_frame.astype(np.float32) * (backward_weight / total)
            )
            result[index] = match_local_color(
                clean.clip(0, 255).astype(np.uint8), crops[index], ring
            )
    return [frame if frame is not None else crops[i] for i, frame in enumerate(result)]
...
def main() -> int:
    ...
    left, top, crop_w, crop_h = crop
    target = 256
    hard, feather, ring = build_masks(crop, box, target, args.alpha_asset)
    ...
    try:
        for scene_start, scene_end in ranges:
            ...
            clean_crops = restore_scene(
                crops, model, mask_tensor, hard, feather, ring, args.anchor_stride
            )
            capture.set(cv2.CAP_PROP_POS_FRAMES, scene_start)
